...
public class TranslateActivity extends AppCompatActivity {
    ...
    public void translateText(View v) {
        ...
        //Code for drop down list in Translate activity
        Spinner spinnerField=(Spinner) findViewById(R.id.transpinner);
        //Code to get the selected value from spinner
        String text = spinnerField.getSelectedItem().toString();

        //Code creating a hash map for language names and their respective codes
        Map<String,String> langMap= new HashMap<String, String>();
        String[] keys={"Arabic","Irish","Hungarian","Urdu","Finnish","French","Korean","Latin","Telugu","Russian","turkish","Chinese","Czech","Dutch","German"};
        String[] values={"ar","ga","hu","ur","fi","fr","ko","la","te","ru","tr","zh","cs","nl","de"};

        for (int i=0;i<keys.length;i++)
        {
            langMap.put(keys[i],values[i]);
        }

        String code=langMap.get(text);
        ...
        String getURL = "https://translate.yandex.net/api/v1.5/tr.json/translate?" +
                ...
                "lang=en-"+code+"&[format=plain]&[options=1]&[callback=set]";//The API service URL
    ...
    {
        //Go to Home page While logging Out
